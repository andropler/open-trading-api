...
> **🎯 최종 Paper 운용 채택 (2026-05-05)**:
> **`kr_5m_composite_mbull2060` + `kr_intraday_breakout_v41`** 두 전략 (50:50 자본 분배 권장).
> 일별 PnL 상관계수 0.053으로 거의 독립적이며, 진입 시간(09~10시 vs 11~12시) + 종목 universe + 청산 스타일이
> 모두 다름. 자세한 근거: [VALIDATION_v2.md §7](./VALIDATION_v2.md).
> ORB(`kr_intraday_orb_5m`)는 baseline 비교용으로만 보존, 운용 제외.

| 모듈 | 전략 ID | 봉 | 진입 스크립트 | Paper 채택 |
|------|---------|---|----------------|:--:|
...
| `intraday_orb.py` | `kr_intraday_orb_5m` | 5m | (예제 없음 — 직접 `IntradayORBBacktester` 호출) | ✗ baseline only |
...
### 핵심 파라미터 (`BreakoutV41Params`) — Paper 운용 디폴트

> **2026-05-05 업데이트**: VALIDATION_v2.md §3 (P3 + F1 + F2) 결과로 v1의 보수 디폴트를
> **F2 best_universe variant**로 교체. TRAIN PF 1.28→**2.20**, TEST PF 1.69→**2.35**,
> TRAIN MDD 12.2%→**7.3%**. Robustness 1.46→**2.27**.

```python
# v1 → v2 변경된 값
breakout_lookback=4    vol_multiplier=3.0    vol_avg_window=20      # vol_mult 2.0→3.0
sl_pct=7.0             trail_pct=0.5         trail_activation=0.5   # sl_pct 5.0→7.0
top_n_stocks=10        ranking_window=5                              # top_n 15→10
...
이전 v1 디폴트(top15/V2.0/SL5.0)는 단일 fit 의심으로 paper 운용에서 제외됨.
